...
class QuickSortTests: XCTestCase {
    ...
    func testAscOdd() {
        let resultArray = QuickSort.sort(TestsHelper.array, order: .ASC)
    ...
    func testDescOdd() {
        let resultArray = QuickSort.sort(TestsHelper.array, order: .DESC)
    ...
    func testNilArray() {
        ...
        let resultArray = QuickSort.sort(nilArray, order: .ASC)

        XCTAssert(resultArray == nil);
    }

    func testEmptyArray() {
        let resultArray = QuickSort.sort([] as [Int], order: .ASC)

        XCTAssert(resultArray ?= []);
    }

    func testArrayWithOneNumber() {
        let resultArray = QuickSort.sort([0], order: .ASC)
