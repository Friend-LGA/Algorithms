...
class BidirectionalBubbleSortTests: XCTestCase {
    ...
    func testAscOdd() {
        let resultArray = BidirectionalBubbleSort.sort(TestsHelper.array, order: .ASC)
    ...
    func testDescOdd() {
        let resultArray = BidirectionalBubbleSort.sort(TestsHelper.array, order: .DESC)
    ...
    func testNilArray() {
        ...
        let resultArray = BidirectionalBubbleSort.sort(nilArray, order: .ASC)

        XCTAssert(resultArray == nil);
    }

    func testEmptyArray() {
        let resultArray = BidirectionalBubbleSort.sort([] as [Int], order: .ASC)

        XCTAssert(resultArray ?= []);
    }
    
    func testArrayWithOneNumber() {
        let resultArray = BidirectionalBubbleSort.sort([0], order: .ASC)
